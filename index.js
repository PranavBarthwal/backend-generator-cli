...
import { packages } from "./snippet-packages.js";
  ...
  .action((snippetName) => {
    try {
      ...
      if (!fs.existsSync(snippetFile)) {
        console.error(
          chalk.red(`Error: Snippet "${snippetName}" does not exist.`)
        );
        return;
      }
      // Define the path where the new snippet file will be created in the working directory
      const newSnippetFilePath = path.join(
        DESTINATION_PATH,
        `${snippetName}.js`
      );
      ...
      exec(
        `npm install ${packages[snippetName].join(" ")}`,
        (err, stdout, stderr) => {
          if (err) {
